...
bool straightFunction(ROVER *rover){
        ...
        if (abs(rover->OFS_y_reading_error) <= 1) {
            ...
            rover->MotorSpeedA = STRAIGHT_PWM_L + rover->PWM_output;
            rover->MotorSpeedB = STRAIGHT_PWM_R - rover->PWM_output;
...
void loop()
{
    ...
    if (mode_ == "M" && client.available() > 5) {
        ...
        //  update target angle
        updateTargets(&rover.B_x, &rover.B_y, &rover.current_x, &rover.current_y, &rover.current_angle, &rover.desired_displacement, &rover.desired_angle);
