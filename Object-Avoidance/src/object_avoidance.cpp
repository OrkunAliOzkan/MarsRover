...
int automation(int * state, double * destination_x, double * destination_y, double current_x, double current_y)
{
    switch(*state){
        ...
        case(1):{
            ...
            *destination_y = ARENA_HEIGHT;
            // /*current_y + */ (ARENA_HEIGHT /*- 2*RADIUS*/); //  have y avoid the walls by a rover width
        ...
        case(3):{
            ...
            *destination_x = current_x;
            *destination_y = 0;
            // /*current_y - */ (0 /*- 2*RADIUS*/); //  have y avoid the walls by a rover width
            // // *destination_y = current_y - (ARENA_HEIGHT /*- 2*RADIUS*/); //  have y avoid the walls by a rover width
            break;
        }
        default:{
...
void loop()
{
    if((mode_ == "A" || mode_ == "AVOID") && (current_x > 0.9*ARENA_WIDTH)){
        // reached the end of the arena, stop
        ...
        // reached next waypoint, get a new one
        ...
        // if reached next waypoint in avoid sequence
        turning_complete = 0;
        straight_line_complete = 0;
